...
def cluster_model_pipeline(DF):
    ...
    if "filled_df" not in st.session_state:
        if st.session_state.contain_null:
            with st.status(
                ...
                attributes, types_info, description_info = contain_null_attributes_info(
                    filled_df
                )
    ...
    if "encoded_df" not in st.session_state:
        if not st.session_state.all_numeric:
            with st.status(
                ...
                non_numeric_attributes, non_numeric_head = non_numeric_columns_and_head(
                    DF
                )
                st.write("Large language model analysis...")
                ...
                DF = encoded_df
                status.update(
                    label="Data encoding completed!", state="complete", expanded=False
                )
            st.download_button(
    ...
    if "df_pca" not in st.session_state:
        ...
        st.session_state.df_pca = perform_PCA_for_clustering(
            st.session_state.data_transformed, n_components
        )
        ...
        with st.container():
            ...
            if not st.session_state["decided_model"]:
                with st.spinner("Deciding models based on data..."):
                    ...
                    model_dict = decide_cluster_model(
                        shape_info, description_info, cluster_info
                    )
...
def display_results(X):
    ...
    with model_col1:
        if "model1_name" not in st.session_state:
            st.session_state.model1_name = get_cluster_method_name(
        ...
        with st.spinner("Model training in progress..."):
            st.session_state.model1 = train_select_cluster_model(
                X, n_clusters1, st.session_state.model_list[0]
            )
    ...
    with model_col2:
        if "model2_name" not in st.session_state:
            st.session_state.model2_name = get_cluster_method_name(
        ...
        with st.spinner("Model training in progress..."):
            st.session_state.model2 = train_select_cluster_model(
                X, n_clusters2, st.session_state.model_list[1]
            )
    ...
    with model_col3:
        if "model3_name" not in st.session_state:
            st.session_state.model3_name = get_cluster_method_name(
        ...
        with st.spinner("Model training in progress..."):
            st.session_state.model3 = train_select_cluster_model(
                X, n_clusters3, st.session_state.model_list[2]
            )
