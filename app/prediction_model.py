...
def prediction_model_pipeline(
    DF,
    API_KEY="r8_f4MP4W51W2lwXi8MLRZMlC0MshE3XU03gnIMi",
    GPT_MODEL="snowflake/snowflake-arctic-instruct",
):
    st.divider()
    ...
    if not st.session_state.target_selected:
        ...
        with st.spinner("AI is analyzing the data..."):
            attributes_for_target, types_info_for_target, head_info_for_target = (
                attribute_info(st.session_state.data_origin)
            )

            st.session_state.target_Y = decide_target_attribute(
                attributes_for_target,
                types_info_for_target,
                head_info_for_target,
                # GPT_MODEL,
                # API_KEY
            )
        ...
        if st.session_state.target_Y != -1:
            ...
            st.info(
                "AI cannot determine the target variable from the data. Please select the target variable"
            )
    ...
    if st.session_state.target_selected:
        ...
        if "contain_null" not in st.session_state:
            st.session_state.contain_null = contains_missing_value(
        ...
        if "filled_df" not in st.session_state:
            if st.session_state.contain_null:
                with st.status(
                    "Processing **missing values** in the data...", expanded=True
                ) as status:
                    st.write("Filtering out high-frequency missing rows and columns...")
                    filled_df = remove_high_null(DF)
                    filled_df = remove_rows_with_empty_target(
                        filled_df, st.session_state.selected_Y
                    )
                    st.write("Large language model analysis...")
                    attributes, types_info, description_info = (
                        contain_null_attributes_info(filled_df)
                    )
                    fill_result_dict = decide_fill_null(
                        attributes, types_info, description_info
                    )
                    ...
                    (
                        ...
                        new_category_list,
                        interpolation_list,
                    ) = separate_fill_null_list(fill_result_dict)
                    ...
                    DF = filled_df
                    status.update(
                        label="Missing value processing completed!",
                        state="complete",
                        expanded=False,
                    )
                st.download_button(
        ...
        if "all_numeric" not in st.session_state:
            st.session_state.all_numeric = check_all_columns_numeric(
                st.session_state.data_origin
            )
        ...
        if "encoded_df" not in st.session_state:
            if not st.session_state.all_numeric:
                with st.status(
                    "Encoding non-numeric data using **numeric mapping** and **one-hot**...",
                    expanded=True,
                ) as status:
                    non_numeric_attributes, non_numeric_head = (
                        non_numeric_columns_and_head(DF)
                    )
                    st.write("Large language model analysis...")
                    ...
                    encoded_df, mappings = convert_to_numeric(
                        DF, convert_int_cols, one_hot_cols, drop_cols
                    )
                    # Store the imputed DataFrame in session_state
                    st.session_state.encoded_df = encoded_df
                    DF = encoded_df
                    status.update(
                        label="Data encoding completed!",
                        state="complete",
                        expanded=False,
                    )
                st.download_button(
                    label="Download Encoded Data",
                    data=st.session_state.encoded_df.to_csv(index=False).encode(
                        "utf-8"
                    ),
        ...
        if "df_cleaned2" not in st.session_state:
            st.session_state.df_cleaned2 = remove_duplicates(
                st.session_state.df_cleaned1
            )
        ...
        if "df_pca" not in st.session_state:
            to_perform_pca, n_components = decide_pca(
                st.session_state.df_cleaned2.drop(columns=[st.session_state.selected_Y])
            )
            ...
            if st.session_state.to_perform_pca:
                st.session_state.df_pca = perform_pca(
                    st.session_state.df_cleaned2,
                    n_components,
        ...
        if "is_binary" not in st.session_state:
            st.session_state["is_binary"] = (
                count_unique(st.session_state.df_pca, st.session_state.selected_Y) == 2
            )
        ...
        if "test_percentage" not in st.session_state:
            with st.spinner("Deciding testing set percentage based on data..."):
                st.session_state.test_percentage = int(
                    decide_test_ratio(st.session_state.df_pca.shape) * 100
                )
        ...
        with balance_column:
            st.metric(
                label="Test Data",
                value=f"{st.session_state.test_percentage}%",
                delta=None,
            )
            st.toggle(
                "Class Balancing",
                value=st.session_state.balance_data,
                key="to_perform_balance",
                on_change=update_balance_data,
                disabled=st.session_state["start_training"],
            )
            st.caption(
                "Strategies for handling imbalanced data sets and to enhance machine learning model performance."
            )
            st.caption(
                "AI will select the most appropriate method to balance the data."
            )
        ...
        if st.session_state["start_training"]:
            with st.container():
                ...
                if (
                    ...
                    and "balance_method" not in st.session_state
                ):
                    with st.spinner(
                        "AI is deciding the balance strategy for the data..."
                    ):
                        (
                            shape_info_balance,
                            description_info_balance,
                            balance_info_balance,
                        ) = get_balance_info(
                            st.session_state.df_pca, st.session_state.selected_Y
                        )
                        st.session_state.balance_method = int(
                            decide_balance(
                                shape_info_balance,
                                description_info_balance,
                                balance_info_balance,
                        ...
                        X_train_res, Y_train_res = check_and_balance(
                            X, Y, method=st.session_state.balance_method
                        )
                else:
                ...
                if not st.session_state.get("data_splitted", False):
                    (
                        ...
                        X_train_res,
                        Y_train_res,
                        st.session_state.test_percentage / 100,
                        42,
                        st.session_state.to_perform_pca,
                    )
                ...
                if not st.session_state["decided_model"]:
                    with st.spinner("Deciding models based on data..."):
                        shape_info, head_info, nunique_info, description_info = (
                            get_data_overview(st.session_state.df_pca)
                        )
                        model_dict = decide_model(
                            shape_info,
                            head_info,
                            nunique_info,
                            description_info,
...
def display_results(X_train, X_test, Y_train, Y_test):
    ...
    with balance_col4:
        st.metric(
            ...
            value=get_balance_method_name(st.session_state.balance_method),
            delta=None,
        )
    ...
    with model_col1:
        if "model1_name" not in st.session_state:
            st.session_state.model1_name = get_model_name(
                st.session_state.model_list[0]
            )
        st.subheader(st.session_state.model1_name)
        with st.spinner("Model training in progress..."):
            if "model1" not in st.session_state:
                st.session_state.model1 = train_selected_model(
                    X_train, Y_train, st.session_state.model_list[0]
                )
                st.session_state.downloadable_model1 = save_model(
                    st.session_state.model1
                )
        # Model metrics
        st.write(
            f"The accuracy of the {st.session_state.model1_name}: ",
            f"\n:green[**{st.session_state.model1.score(X_test, Y_test)}**]",
        )
        st.pyplot(
            confusion_metrix(
                st.session_state.model1_name, st.session_state.model1, X_test, Y_test
            )
        )
        st.write(
            "F1 Score: ",
            f":green[**{calculate_f1_score(st.session_state.model1, X_test, Y_test, st.session_state.is_binary)}**]",
        )
        if st.session_state.model_list[0] != 2 and st.session_state["is_binary"]:
            ...
            st.pyplot(
                roc(
                    ...
                    st.session_state.fpr1,
                    st.session_state.tpr1,
                )
            )
            st.write(
                f"The AUC of the {st.session_state.model1_name}: ",
                f"\n:green[**{auc(st.session_state.fpr1, st.session_state.tpr1)}**]",
            )

    with model_col2:
        if "model2_name" not in st.session_state:
            st.session_state.model2_name = get_model_name(
                st.session_state.model_list[1]
            )
        st.subheader(st.session_state.model2_name)
        with st.spinner("Model training in progress..."):
            if "model2" not in st.session_state:
                st.session_state.model2 = train_selected_model(
                    X_train, Y_train, st.session_state.model_list[1]
                )
                st.session_state.downloadable_model2 = save_model(
                    st.session_state.model2
                )
        # Model metrics
        st.write(
            f"The accuracy of the {st.session_state.model2_name}: ",
            f"\n:green[**{st.session_state.model2.score(X_test, Y_test)}**]",
        )
        st.pyplot(
            confusion_metrix(
                st.session_state.model2_name, st.session_state.model2, X_test, Y_test
            )
        )
        st.write(
            "F1 Score: ",
            f":green[**{calculate_f1_score(st.session_state.model2, X_test, Y_test, st.session_state.is_binary)}**]",
        )
        if st.session_state.model_list[1] != 2 and st.session_state["is_binary"]:
            ...
            st.pyplot(
                roc(
                    ...
                    st.session_state.fpr2,
                    st.session_state.tpr2,
                )
            )
            st.write(
                f"The AUC of the {st.session_state.model2_name}: ",
                f"\n:green[**{auc(st.session_state.fpr2, st.session_state.tpr2)}**]",
            )

    with model_col3:
        if "model3_name" not in st.session_state:
            st.session_state.model3_name = get_model_name(
                st.session_state.model_list[2]
            )
        st.subheader(st.session_state.model3_name)
        with st.spinner("Model training in progress..."):
            if "model3" not in st.session_state:
                st.session_state.model3 = train_selected_model(
                    X_train, Y_train, st.session_state.model_list[2]
                )
                st.session_state.downloadable_model3 = save_model(
                    st.session_state.model3
                )
        # Model metrics
        st.write(
            f"The accuracy of the {st.session_state.model3_name}: ",
            f"\n:green[**{st.session_state.model3.score(X_test, Y_test)}**]",
        )
        st.pyplot(
            confusion_metrix(
                st.session_state.model3_name, st.session_state.model3, X_test, Y_test
            )
        )
        st.write(
            "F1 Score: ",
            f":green[**{calculate_f1_score(st.session_state.model3, X_test, Y_test, st.session_state.is_binary)}**]",
        )
        if st.session_state.model_list[2] != 2 and st.session_state["is_binary"]:
            ...
            st.pyplot(
                roc(
                    st.session_state.model3_name,
                    st.session_state.fpr3,
                    st.session_state.tpr3,
                )
            )
            st.write(
                f"The AUC of the {st.session_state.model3_name}: ",
