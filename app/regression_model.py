...
def regression_model_pipeline(DF):
    st.divider()
    ...
    if st.session_state.target_selected:
        ...
        if "data_transformed" not in st.session_state:
            st.session_state.data_transformed = transform_data_for_clustering(
        ...
        with balance_column:
            ...
            st.toggle(
                "Class Balancing", value=False, key="to_perform_balance", disabled=True
            )
        ...
        if st.session_state["start_training"]:
            with st.container():
                st.header("Modeling")
                X_train_res, Y_train_res = select_Y(
                ...
                if not st.session_state["decided_model"]:
                    with st.spinner("Deciding models based on data..."):
                        shape_info, _, _, description_info = get_data_overview(
...
def display_results(X_train, X_test, Y_train, Y_test):
    ...
    with model_col1:
        if "model1_name" not in st.session_state:
            st.session_state.model1_name = get_regression_method_name(
        ...
        st.write(
            "R2 Score: ",
            f":green[**{calculate_r2_score(st.session_state.y_pred1, Y_test)}**]",
        )
        ...
        st.write(
            "Mean Absolute Error: ",
            f":green[**{calculate_mae(st.session_state.y_pred1, Y_test)}**]",
        )
        st.pyplot(plot_qq_plot(st.session_state.y_pred1, Y_test))

    with model_col2:
        if "model2_name" not in st.session_state:
            st.session_state.model2_name = get_regression_method_name(
        ...
        st.write(
            "R2 Score: ",
            f":green[**{calculate_r2_score(st.session_state.y_pred, Y_test)}**]",
        )
        ...
        st.write(
            "Mean Absolute Error: ",
            f":green[**{calculate_mae(st.session_state.y_pred, Y_test)}**]",
        )
        st.pyplot(plot_qq_plot(st.session_state.y_pred, Y_test))

    with model_col3:
        if "model3_name" not in st.session_state:
            st.session_state.model3_name = get_regression_method_name(
        ...
        st.write(
            "R2 Score: ",
            f":green[**{calculate_r2_score(st.session_state.y_pred3, Y_test)}**]",
        )
        ...
        st.write(
            "Mean Absolute Error: ",
            f":green[**{calculate_mae(st.session_state.y_pred3, Y_test)}**]",
        )
